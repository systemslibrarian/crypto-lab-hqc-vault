 ...
 *    files the node under `incomplete`. `body` runs a radial gradient behind the
 *    whole document, the `.btn` fills are `linear-gradient`s, and 42 sites mix
 *    accents into surfaces with `color-mix()`, so a large share of this page's
 *    text sits on something axe will not measure.
 ...
 *    `opacity: 0.6`, and `.cl-hero-sub` renders at `opacity: .85`.
 *
 ...
 *    comparison, decoding-budget, verifier and sizing tables sit in
 *    `overflow-x: auto` wraps — the widest is `min-width: 980px` — so at 380px
 *    most of their columns are clipped away. Content scrolled past the client box is not dimmed or partly
 ...
 *    quasi-cyclic and concatenated-code figures are `<svg>`: each draws a shape
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * (the chips, the TOC active state, the callouts, the button fills — 42
     ...
     * that here so a deliberately dimmed disabled control — a `.btn:disabled`
     * at `opacity: 0.6` while a sweep is running — is not reported as a failure
    ...
    for (const el of Array.from(document.querySelectorAll('body *'))) {
      ...
      const cs = styleOf(el);
      // SVG text takes its ink from `fill`, not `color`. The code-structure
      // figures label their parts with `<text>`, and reading `color` there
